...
def index(request):
    ...
    if request.method == "POST" and 'edit' in request.POST:
        ...
        person.이름 =request.POST['name']
        person.나이 = request.POST['age']
        person.직급 = request.POST['position']
        person.부서 = request.POST['department']
        person.save()
    ...
    if request.method == "POST" and 'add' in request.POST:
        ...
        person.직급 = request.POST['position']
        person.부서 = request.POST['department']
        person.Rank = request.POST['rank']
        person.status=1
        person.save()
        return redirect('index')
    person_list = Person.objects.filter(status=1)
